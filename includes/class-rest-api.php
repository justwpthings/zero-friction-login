...
class ZFL_REST_API {
    ...
    public function request_auth($request) {
        ...
        if ($is_registration) {
            set_transient('zfl_display_name_' . md5($email), sanitize_text_field($display_name), 1800);
    ...
    private function get_redirect_url() {
        ...
        switch ($redirect_setting) {
            ...
            case 'custom_url':
                $custom_url = get_option('zfl_custom_login_url', '');
    ...
    private function get_logout_redirect_url($use_referer_for_same_page = true) {
        ...
        switch ($redirect_setting) {
            case 'home':
                return home_url();
            case 'custom_url':
                $custom_url = get_option('zfl_custom_logout_url', '');
                return !empty($custom_url) ? $custom_url : home_url();
            case 'same_page':
