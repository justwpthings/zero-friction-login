...
export const App: React.FC = () => {
  ...
  useEffect(() => {
    if (config) {
      console.log('[ZFL] is_logged_in:', config.is_logged_in);
      console.log('[ZFL] current_user:', config.current_user);
  ...
  const loadConfig = async () => {
    try {
      console.log('[ZFL] Loading configuration...');
      const configData = await getConfig();
      console.log('[ZFL] Config loaded:', configData);
      setConfig(configData);
      setViewState('login');
    } catch (error) {
      console.error('[ZFL] Failed to load config:', error);
